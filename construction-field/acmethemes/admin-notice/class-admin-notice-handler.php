<?php
/**
 * Construction Field Notice Handler
 *
 * @author  AcmeThemes
 ...
 *
 * Class Construction_Field_Notice_Handler
 */
class Construction_Field_Notice_Handler {
	...
	public function replace_term_ids( $replace_term_ids ){
		...
		/*Terms IDS*/
		$term_ids = array(
            'construction_field_post_cat',
            'construction_field_post_tag',
            'construction_field_cat_id',
	...
	public function admin_notice() {
		...
		if ( ! $notice_nag ) {
			wp_enqueue_style( 'construction-field-notice', get_template_directory_uri() . '/acmethemes/admin-notice/admin-notice.css', array(), '3.0.0' );
			wp_enqueue_script( 'construction-field-adi-install', get_template_directory_uri()  . '/acmethemes/admin-notice/admin-notice.js', array( 'jquery' ), '', true );

			$translation = array(
			        'btn_text' => esc_html__( 'Processing...', 'construction-field' ),
                    'nonce'    => wp_create_nonce( 'construction-field-demo-import-nonce' )
            );
			wp_localize_script( 'construction-field-adi-install', 'construction_field_adi_install', $translation );
	...
	public static function hide_notices() {
		...
		if ( isset( $_GET['at-gsm-hide-notice'] ) && isset( $_GET['at_gsm_admin_notice_nonce'] ) ) { // WPCS: input var ok.
			if ( ! wp_verify_nonce( wp_unslash( $_GET['at_gsm_admin_notice_nonce'] ), 'at_gsm_hide_notices_nonce' ) ) { // phpcs:ignore WordPress.VIP.ValidatedSanitizedInput.InputNotSanitized
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'construction-field' ) ); // WPCS: xss ok.
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'construction-field' ) ); // WPCS: xss ok.
			}

			$notice_type = sanitize_text_field( wp_unslash( $_GET['at-gsm-hide-notice'] ) );
			update_option( 'construction_field_admin_notice_'. $notice_type, 1 );

			/*Update to Hide.*/
			if ( 'welcome' === $_GET['at-gsm-hide-notice'] ) {
				update_option( 'construction_field_admin_notice_'. $notice_type, 1 );
			} else { // Show.
				delete_option( 'construction_field_admin_notice_'. $notice_type );
			}
		}
	...
	public function getting_started() {
        ...
        <div id="at-gsm" class="updated notice-info at-gsm">
            <a class="at-gsm-close notice-dismiss" href="<?php echo esc_url( wp_nonce_url( remove_query_arg( array( 'activated' ), add_query_arg( 'at-gsm-hide-notice', 'welcome' ) ), 'at_gsm_hide_notices_nonce', 'at_gsm_admin_notice_nonce' ) ); ?>">
				<?php esc_html_e( 'Dismiss', 'construction-field' ); ?>
            </a>
            <div class="at-gsm-container">
                <img class="at-gsm-screenshot" src="<?php echo esc_url(get_template_directory_uri().'/screenshot.jpg' )?>" alt="<?php esc_html_e( 'Construction Field', 'construction-field' ); ?>" />
                <div class="at-gsm-notice">
                    <h2>
						<?php
						printf(
						/* translators: 1: welcome page link starting html tag, 2: welcome page link ending html tag. */
							esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer, please make sure you visit our %2$swelcome page%3$s.', 'construction-field' ), '<strong>'. wp_get_theme()->get('Name'). '</strong>','<a href="' . esc_url( admin_url( 'themes.php?page=construction-field-info' ) ) . '">','</a>' );
						?>
                    </h2>

                    <p class="plugin-install-notice"><?php esc_html_e( 'Clicking the button below will install and activate the Advanced Import plugin.', 'construction-field' ); ?></p>

                    <a class="at-gsm-btn button button-primary button-hero" href="#" data-name="" data-slug="" aria-label="<?php esc_html_e( 'Get started with Construction Field', 'construction-field' ); ?>">
		                <?php esc_html_e( 'Get started with Construction Field', 'construction-field' );?>
                    </a>
                </div>
            </div>
        </div>
	...
	public function install_advanced_import() {
		...
		check_ajax_referer( 'construction-field-demo-import-nonce', 'security' );
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			$status['errorMessage'] = __( 'Sorry, you are not allowed to install plugins on this site.', 'construction-field' );
		...
		if ( is_wp_error( $result ) ) {
			...
			$status['errorMessage'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'construction-field' );
	...
	public function setup_review_notice() {
		...
		// Set the installed time in `construction_field_theme_installed_time` option table.
		$option = get_option('construction_field_theme_installed_time' );

		if ( ! $option ) {
			update_option('construction_field_theme_installed_time', time() );
		}
	...
	public function display_review_notice() {
		 ...
		 */
		if ( ( get_option('construction_field_theme_installed_time' ) > strtotime( '-15 day' ) ) || ( $ignored_notice_partially > strtotime( '-15 day' ) ) || ( $ignored_notice ) ) {
			return;
		}
		?>
		<div class="notice updated at-review-notice">
			<p>
				...
				/* Translators: %1$s current user display name. */
					esc_html__(
						'Howdy, %1$s! It seems that you have been using this theme for more than 15 days. We hope you are happy with everything that the theme has to offer. If you can spare a minute, please help us by leaving a 5-star review on WordPress.org.  By spreading the love, we can continue to develop new amazing features in the future, for free!', 'construction-field'
					),
			...
			<div class="links">
				<a href="https://wordpress.org/support/theme/construction-field/reviews/?filter=5#new-post" class="btn button-primary" target="_blank">
					<span class="dashicons dashicons-thumbs-up"></span>
					<span><?php esc_html_e( 'Sure', 'construction-field' ); ?></span>
				</a>

				<a href="?nag_remove_theme_review_notice_partially=0" class="btn at-danger-btn">
					<span class="dashicons dashicons-calendar"></span>
					<span><?php esc_html_e( 'Maybe later', 'construction-field' ); ?></span>
				</a>

				<a href="?nag_remove_theme_review_notice=0" class="btn at-success-btn">
					<span class="dashicons dashicons-smiley"></span>
					<span><?php esc_html_e( 'I already did', 'construction-field' ); ?></span>
				</a>

				<a href="<?php echo esc_url( 'https://www.acmethemes.com/supports/forum/construction-field/' ); ?>" class="btn at-default-btn" target="_blank">
					<span class="dashicons dashicons-edit"></span>
					<span><?php esc_html_e( 'Got theme support question?', 'construction-field' ); ?></span>
				</a>
			</div>
	...
	public function remove_review_notice() {
		...
		global $current_user;
		$user_id                  = $current_user->ID;
		$theme_installed_time     = get_option('construction_field_theme_installed_time' );
		$ignored_notice           = get_user_meta( $user_id, 'remove_theme_review_notice', true );
		$ignored_notice_partially = get_user_meta( $user_id, 'nag_remove_theme_review_notice_partially', true );

		// Delete options data.
		if ( $theme_installed_time ) {
			delete_option('construction_field_theme_installed_time' );
		}
	...
	public function review_notice_enqueue() {
		...
		wp_enqueue_style( 'construction-field-review-notice', get_template_directory_uri()  . '/acmethemes/admin-notice/admin-notice.css' );
 ...
 * @since    1.0.0
 */
function construction_field_notice_handler( ) {
	return Construction_Field_Notice_Handler::instance();
}
construction_field_notice_handler()->run();


/*For this theme only*/
if( !function_exists( 'construction_field_ai_update_image_size') ){
	function construction_field_ai_update_image_size(){
		/*Thumbnail Size*/
...
add_action( 'advanced_import_before_content_screen', 'construction_field_ai_update_image_size' );